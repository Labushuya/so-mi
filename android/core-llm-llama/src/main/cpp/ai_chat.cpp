// so-mi local override of upstream ai_chat.cpp (v0.11.2).
//
// The upstream sample at
//   upstream/examples/llama.android/lib/src/main/cpp/ai_chat.cpp
// is referenced verbatim by every other consumer of llama.android. We
// override only DEFAULT_CONTEXT_SIZE here, from 8192 to 4096, because:
//
//   - On Magic V2's CPU-only Q4_K_M 7B path, soul.md prefill at 8K KV
//     cache routinely exceeds the 5 s ForegroundService grace window
//     and the OS kills the process mid-load (the ANR dialog briefly
//     appears then auto-dismisses — that is the v0.11.1 freeze bug).
//   - 4K halves both the KV-cache RAM footprint and prefill latency.
//   - Phase 3 (RAG) is not yet wired, so 8K context is currently
//     unused capacity. When RAG lands and the budget heuristic is
//     re-tuned, this value bumps back up in lockstep.
//
// Do NOT edit upstream/.../ai_chat.cpp - a `git submodule update`
// would clobber the patch. CMakeLists.txt resolves this file via
// CMAKE_CURRENT_LIST_DIR/ai_chat.cpp in preference to the upstream
// path.
